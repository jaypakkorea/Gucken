  ...
  <div class="SearchFlexDiv">
    <div class="UserLeftDiv">
      <div style="position:relative;">
        ...
        ></b-avatar>
        <div
          v-if="this.$store.state.currentUser.pk === profile.id && !profile.profile_pic"
          style="position:absolute; left: 40%; top:70% ; color:white; font-weight: bold;"
        >C l i c k !</div>
      </div>
      ...
      <div class="userIconDiv">
        <div v-if="profile.follower_count < 5" id="tooltip-target-1">
          <font-awesome-icon icon="fa-solid fa-trophy" />
        </div>
        <div v-if="profile.follower_count >= 5" id="tooltip-target-1" style="color:#ffc107;">
          <font-awesome-icon icon="fa-solid fa-trophy" />
        </div>

        <div v-if="likeCount < 5" id="tooltip-target-2">
          <font-awesome-icon icon="fa-solid fa-medal" />
        </div>
        <div v-if="likeCount >= 5" id="tooltip-target-2" style="color:#ffc107;">
          <font-awesome-icon icon="fa-solid fa-medal" />
        </div>

        <div v-if="profile.ratings_count < 5" id="tooltip-target-3">
          <font-awesome-icon icon="fa-solid fa-award" />
        </div>
        <div v-if="profile.ratings_count >= 5" id="tooltip-target-3" style="color:#ffc107;">
          <font-awesome-icon icon="fa-solid fa-award" />
        </div>
      ...
      <div v-if="(this.$store.state.currentUser.pk !== profile.id)">
        <b-button
          v-if="follow_follwing"
          variant="warning"
          class="followButton"
          @click="followIng"
        >FOLLOW</b-button>
      ...
      <b-tabs content-class="mt-3">
        <b-tab title="PROFILE" active>
          <div class="userProfileDiv" v-if="correctState">
            ...
            <div
              class="userText"
              style="cursor : pointer; font-family: BMJUA;"
              @click="FollowerStateChange"
            >{{profile.follower_count}}</div>
            ...
            >{{profile.date_joined.split('T')[0].replace(/-/g,' / ')}}</div>
          </div>
        </b-tab>
